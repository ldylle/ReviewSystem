...
ssize_t FileSystem::write_file(const std::string& path, const std::vector<uint8_t>& data,
    ...
    // [FIX] 强制同步（因为分配了块，修改了 Block Bitmap）
    sync();
...
int FileSystem::find_inode_by_path(const std::string& path) {
    ...
    for (const auto& part : parts) {
        ...
        if (!found) {
            std::cerr << "[FS] Error: Component '" << part << "' not found in path: " << path << std::endl;
            std::cerr << "[FS]   Searching in inode " << current_inode << ", found " << entries.size() << " entries:" << std::endl;
            for (const auto& e : entries) {
                std::cerr << "[FS]     - " << e.name << " (inode " << e.inode_number << ")" << std::endl;
            }
            return -1;
        }
    }
...
bool FileSystem::restore_backup(const std::string& backup_path) {
    ...
    disk_.open(disk_file_, std::ios::binary | std::ios::in | std::ios::out);
    return disk_.is_open() && read_super_block();
}
